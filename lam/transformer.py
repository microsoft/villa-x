...
class AttnBlock(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        self.embed_dim, self.n_heads, self.do_layer_scale = (
            embed_dim,
            n_heads,
            do_layer_scale,
        )
        ...
        self.mlp = nn.Sequential(
            ...
            (
                ...
                if do_swish_glu
                    ...
                    nn.Linear(embed_dim, int(mlp_ratio * embed_dim)), nn.GELU()
                )
            ),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        if self.do_layer_scale:
            q = q + self.layer_scale_attn(
                self.attn(
                    q_in=self.pre_norm_attn(q),
                    kv_in=self.pre_norm_attn(kv),
                )
            )
